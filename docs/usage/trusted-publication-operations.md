...
Until those policies are approved by a separate Issue/ADR, the current system
...
The authoritative clean Compose/PostgreSQL run is owned by
[NeKiro-Stack](https://github.com/NeKiro-project/NeKiro-Stack). Its pinned
backend acceptance proves the positive cross-runtime lineage, every failure
row above, exact Release provenance, secret absence, sanitized log capture,
and volume cleanup in the same isolated run.
