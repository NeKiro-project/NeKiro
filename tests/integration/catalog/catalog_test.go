...
func TestCatalogPostgreSQLAndHTTPAcceptance(t *testing.T) {
	...
	t.Run("fixed authentication and registration semantics", func(t *testing.T) {
		missing := request(t, http.MethodGet, server.baseURL+"/v1/agents", "", nil)
		...
		draft := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, runtimeA))
		...
		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, runtimeA)), http.StatusConflict, contracts.ErrorCodeConflict)
...
WHERE v.agent_id = 'runtime-a' AND v.version = '1.0.0'`).Scan(&originalCard, &originalOwner); err != nil {
		...
		crossOwnerConflict := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerBToken, registrationEnvelope(t, mustJSON(t, crossOwnerExact)))
...
WHERE v.agent_id = 'runtime-a' AND v.version = '1.0.0'`).Scan(&retainedCard, &retainedOwner); err != nil {
		...
		boundary := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, mustJSON(t, boundaryCard)))
...
WHERE agent_id = 'unbounded-number-agent' AND version = '1.0.0'`).Scan(
		...
		if result := request(t, http.MethodPost, server.baseURL+"/v1/agents/unbounded-number-agent/versions/1.0.0/publish", ownerAToken, nil); result.status != http.StatusOK {
			t.Fatalf("publish unbounded number Card = %d %s", result.status, result.body)
		}
		boundaryDiscovery := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?query=PostgreSQL&capability=number.boundary", userToken, nil))
		...
		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, invalid)), http.StatusBadRequest, contracts.ErrorCodeValidationError)

		crossOwner := decodeCard(t, runtimeA)
		crossOwner.Version = "2.0.0"
		crossOwner.Owner.ID = "catalog-owner-b"
		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerBToken, registrationEnvelope(t, mustJSON(t, crossOwner))), http.StatusForbidden, contracts.ErrorCodeForbidden)

		assertPlatformError(t, request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", userToken, nil), http.StatusForbidden, contracts.ErrorCodeForbidden)
		if result := request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", ownerAToken, nil); result.status != http.StatusOK {
	...
	t.Run("publication discovery disablement and cross-runtime metadata", func(t *testing.T) {
		publishedA := request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/publish", ownerAToken, nil)
		...
		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/publish", ownerAToken, nil), http.StatusConflict, contracts.ErrorCodeConflict)
		if result := request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", userToken, nil); result.status != http.StatusOK {
			t.Fatalf("published public read = %d %s", result.status, result.body)
		}

		if result := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerBToken, registrationEnvelope(t, runtimeB)); result.status != http.StatusCreated {
			t.Fatalf("register Runtime B = %d %s", result.status, result.body)
		}
		if result := request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-b/versions/1.0.0/publish", ownerBToken, nil); result.status != http.StatusOK {
			t.Fatalf("publish Runtime B = %d %s", result.status, result.body)
		}
		search := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=runtime.echo", userToken, nil))
		if len(search.Items) != 2 {
			t.Fatalf("cross-runtime discovery count = %d, want 2", len(search.Items))
		}
		filtered := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?query=translation&capability=runtime.translate&ownerId=catalog-owner-b", userToken, nil))
		...
		for _, literal := range []string{"%", "_"} {
			literalResult := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?query="+url.QueryEscape(literal), userToken, nil))
			if len(literalResult.Items) != 0 {
				t.Fatalf("literal wildcard query %q matched %d rows", literal, len(literalResult.Items))
			}
		}

		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/disable", userToken, nil), http.StatusForbidden, contracts.ErrorCodeForbidden)
		firstDisable := request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/disable", ownerAToken, nil)
		secondDisable := request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/disable", ownerAToken, nil)
		...
		disabledOwnerRead := request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", ownerAToken, nil)
		...
		assertPlatformError(t, request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", userToken, nil), http.StatusForbidden, contracts.ErrorCodeForbidden)
		assertPlatformError(t, request(t, http.MethodPost, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0/publish", ownerAToken, nil), http.StatusConflict, contracts.ErrorCodeConflict)
		disabledAfterRepublishAttempt := decodeEntry(t, request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-a/versions/1.0.0", ownerAToken, nil))
		...
		afterDisable := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=runtime.echo", userToken, nil))
	...
	t.Run("concurrent lifecycle has one legal final state", func(t *testing.T) {
		...
		if result := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, mustJSON(t, card))); result.status != http.StatusCreated {
		...
		go func() {
			defer wait.Done()
			<-start
			statuses <- request(t, http.MethodPost, server.baseURL+"/v1/agents/race-agent/versions/1.0.0/publish", ownerAToken, nil).status
		}()
		go func() {
			defer wait.Done()
			<-start
			statuses <- request(t, http.MethodPost, server.baseURL+"/v1/agents/race-agent/versions/1.0.0/disable", ownerAToken, nil).status
		}()
		close(start)
		wait.Wait()
		close(statuses)
		...
		final := decodeEntry(t, request(t, http.MethodGet, server.baseURL+"/v1/agents/race-agent/versions/1.0.0", ownerAToken, nil))
	...
	t.Run("concurrent duplicate registration is atomic", func(t *testing.T) {
		...
		for range callers {
			go func() {
				defer wait.Done()
				<-start
				statuses <- request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, body).status
			}()
		}
		close(start)
		wait.Wait()
	...
	t.Run("restart and explicit dependency failure", func(t *testing.T) {
		...
		server = startServer(t, root, databaseURL, binary)
		if result := request(t, http.MethodGet, server.baseURL+"/v1/agents/runtime-b/versions/1.0.0", userToken, nil); result.status != http.StatusOK {
			t.Fatalf("durable read after restart = %d %s", result.status, result.body)
		}
		boundaryRead := decodeEntry(t, request(t, http.MethodGet, server.baseURL+"/v1/agents/unbounded-number-agent/versions/1.0.0", userToken, nil))
		if got := boundaryRead.Card.Limits.MaxInputBytes.String(); got != "1e1000001" {
			t.Fatalf("unbounded number after restart = %s", got)
		}
		boundaryDiscovery := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=number.boundary", userToken, nil))
		...
		failure := request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=runtime.echo", userToken, nil)
	...
	t.Run("stable cursor traversal and scale latency", func(t *testing.T) {
		...
		go func() {
			secondPublication <- request(t, http.MethodPost, server.baseURL+"/v1/agents/clock-race-b/versions/1.0.0/publish", ownerAToken, nil)
		}()
		select {
		...
		first := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=scale.test&limit=100", userToken, nil))
		if len(first.Items) != 100 || first.NextCursor == nil {
			t.Fatalf("first scale page = %d items, cursor %v", len(first.Items), first.NextCursor)
		}
		mismatched := request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=other.test&limit=100&cursor="+*first.NextCursor, userToken, nil)
		...
		for cursor != nil {
			page := decodeSearch(t, request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=scale.test&limit=100&cursor="+*cursor, userToken, nil))
		...
		started := time.Now()
		page := request(t, http.MethodGet, server.baseURL+"/v1/agents?capability=scale.test&limit=100", userToken, nil)
...
func startServer(t *testing.T, root, databaseURL, binary string) *testServer {
	...
	server.command.Env = environmentWith(map[string]string{
		...
		"NEKIRO_ROUTER_INTERNAL_URL":                      "http://router-integration:8081/internal/v1/invocations",
...
func registerClockRaceCards(t *testing.T, server *testServer) {
	...
	for _, agentID := range []string{"clock-race-a", "clock-race-b"} {
		...
		result := request(t, http.MethodPost, server.baseURL+"/v1/agents", ownerAToken, registrationEnvelope(t, mustJSON(t, card)))
