...
func TestLoadInvocationRuntimeRequiresExactNoDefaultConfiguration(t *testing.T) {
	...
	if loaded.RouterInternalURL != "http://router.test:8081/internal/v1/invocations" || loaded.RouterBearerToken != "router-secret" || loaded.InternalRequestLimitBytes != 1048576 || loaded.PublicRequestLimitBytes != 1048576 || loaded.SSEEventLimitBytes != 65536 || loaded.MetadataResponseLimitBytes != 1048576 || loaded.DeadlineMS != 30000 {
...
func TestLoadInvocationRuntimeRejectsInvalidDestinationSecretAndNumbers(t *testing.T) {
	tests := []struct{ name, variable, value string }{
		{"relative URL", "NEKIRO_ROUTER_INTERNAL_URL", "/internal/v1/invocations"},
		{"wrong path", "NEKIRO_ROUTER_INTERNAL_URL", "http://router.test:8081/internal/v1/other"},
		{"retired v4 path", "NEKIRO_ROUTER_INTERNAL_URL", "http://router.test:8081/internal/v4/invocations"},
		{"URL credentials", "NEKIRO_ROUTER_INTERNAL_URL", "http://user:secret@router.test:8081/internal/v1/invocations"},
		{"URL query", "NEKIRO_ROUTER_INTERNAL_URL", "http://router.test:8081/internal/v1/invocations?target=other"},
...
func setValidInvocationRuntime(t *testing.T) {
	t.Helper()
	t.Setenv("NEKIRO_ROUTER_INTERNAL_URL", "http://router.test:8081/internal/v1/invocations")
