...
type NonStreamingTransport interface {
	ValidateNonStreamingTarget(contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) error
	SendNonStreaming(context.Context, contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) (json.RawMessage, error)
	ValidateNonStreamingInput(contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) error
}

type StreamingTransport interface {
	SendStreaming(context.Context, contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) iter.Seq2[streammodel.Event, error]
	ValidateStreamingTarget(contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) error
	ValidateStreamingInput(contracts.DispatchInvocationRequestV1, contracts.ResolveAgentResponse) error
}
...
func (handler *DispatchHandler) RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("POST /internal/v1/invocations", handler.dispatch)
}
...
func (handler *DispatchHandler) DispatchChild(writer http.ResponseWriter, request *http.Request, dispatchRequest contracts.DispatchInvocationRequestV1, accept string) {
...
func (handler *DispatchHandler) readRequest(request *http.Request) (contracts.DispatchInvocationRequestV1, error) {
	...
	if err != nil {
		return contracts.DispatchInvocationRequestV1{}, err
	}
	...
	var value contracts.DispatchInvocationRequestV1
...
func validateDispatch(value contracts.DispatchInvocationRequestV1) error {
...
func validateChildDispatch(value contracts.DispatchInvocationRequestV1) error {
	for _, identifier := range []string{value.InvocationID, value.RootTaskID, value.WorkspaceID, value.TargetAgentID, value.Capability, value.Caller.ID} {
		if !validIdentifier(identifier) {
...
func validateResolvedReleaseProvenance(request contracts.DispatchInvocationRequestV1, resolved contracts.ResolveAgentResponse) error {
...
func (handler *DispatchHandler) writeCorrelatedError(writer http.ResponseWriter, request contracts.DispatchInvocationRequestV1, code contracts.PlatformErrorCode) {
	status := errorStatus(code)
...
func (handler *DispatchHandler) writeInvocationResult(writer http.ResponseWriter, request contracts.DispatchInvocationRequestV1, result json.RawMessage) {
...
func (handler *DispatchHandler) dispatchNonStreamingWithLedger(ctx context.Context, writer http.ResponseWriter, request contracts.DispatchInvocationRequestV1, resolved contracts.ResolveAgentResponse, targetErr error) {
...
func (handler *DispatchHandler) dispatchStreamingWithLedger(ctx context.Context, cancel context.CancelFunc, response http.ResponseWriter, request contracts.DispatchInvocationRequestV1, resolved contracts.ResolveAgentResponse) {
	startedAt := time.Now().UTC().Truncate(time.Microsecond)
	childMode := request.ParentInvocationID != ""
...
func (handler *DispatchHandler) appendInitialLedgerEventsMode(ctx context.Context, writer http.ResponseWriter, request contracts.DispatchInvocationRequestV1, startedAt time.Time, events []contracts.InvocationEventV03, childMode bool) bool {
...
func (handler *DispatchHandler) appendStreamingTerminal(ctx context.Context, request contracts.DispatchInvocationRequestV1, sequence int64, startedAt time.Time, latency int64, code contracts.PlatformErrorCode, eventType, status string) error {
...
func (handler *DispatchHandler) finishStreamingFailure(ctx context.Context, cancel context.CancelFunc, writer *resultStreamWriter, sequence *contracts.RuntimeResultStreamSequenceValidator, request contracts.DispatchInvocationRequestV1, startedAt time.Time, streamSequence, ledgerSequence int64, code contracts.PlatformErrorCode) {
...
func (handler *DispatchHandler) finishStreamingFailureWithoutLedger(ctx context.Context, cancel context.CancelFunc, writer *resultStreamWriter, sequence *contracts.RuntimeResultStreamSequenceValidator, request contracts.DispatchInvocationRequestV1, streamSequence int64, code contracts.PlatformErrorCode) {
	typeValue, status := streamFailureType(code)
	event, err := streamFailureEvent(request, streamSequence, typeValue, status, code)
	if err != nil {
		cancel()
...
func streamFailureEvent(request contracts.DispatchInvocationRequestV1, sequence int64, eventType contracts.ResultStreamEventType, status string, code contracts.PlatformErrorCode) (contracts.InvocationResultStreamEventV2, error) {
...
func lifecycleEvent(request contracts.DispatchInvocationRequestV1, sequence int64, eventType, status string, occurredAt time.Time) contracts.InvocationEventV03 {
...
func terminalLifecycleEvent(request contracts.DispatchInvocationRequestV1, sequence int64, eventType, status string, occurredAt time.Time, latencyMS int64, code contracts.PlatformErrorCode) (contracts.InvocationEventV03, error) {
