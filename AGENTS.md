...
at specs/018-invocation-trace-reads/plan.md
<!-- SPECKIT END -->
...
当前仓库已将跨边界契约事实来源迁移为语言无关的 JSON Schema、OpenAPI 和 A2A Profile。当前活动契约集为 Agent Card `0.2`、Workspace `1`、Installation `2`、Catalog/Northbound API `v3`、Control Plane Internal API `v2`、Northbound Invocation API `v4`、Router Internal API `v3`、Router Agent API `v1`、Invocation Event `0.3`、Platform Error `v4`（运行时）与 `v3`（Workspace/Installation/内部解析）、Invocation Result `v1` / Result Stream Event `v2`，以及 A2A Profile Schema `0.2` / protocol `0.3.0`；历史工件仅作为迁移证据保留，不提供运行时双读 fallback。

当前仓库已完成 Spec 002 的 Catalog、Spec 003/004/005/006/007/008/009 的 Workspace 与 Installation runtime，并在当前分支完成 Spec 012 Control Plane Invocation Dispatch、Spec 013 A2A Router Foundation、Spec 014 Router-owned Invocation Ledger、Spec 015 Runtime B direct A2A sample、Spec 016 non-streaming dispatch、Spec 017 streaming A2A delivery，以及 Spec 018 Invocation/Trace metadata reads。已落地的运行时边界包括 Workspace owner policy、精确解析、Gateway v4 调用、Router-mediated JSON/SSE 调用、metadata-only Ledger 和 Workspace-scoped Invocation/Trace 查询。Frontend 开发保持暂停；Agent SDK、Runtime A、跨 Runtime 嵌套调用、完整 Compose/E2E 验收闭环和最终部署集成仍未实现，不得因现有 runtime slice 或 active contracts 而宣称这些后续能力存在。本次状态更新原因是 Spec 012 与 Spec 018 已在当前分支实现并通过本地测试/竞态验证，影响是后续实现必须复用现有 Workspace/Catalog ports、Router Ledger read port 和 active v4/v3 runtime contracts。
